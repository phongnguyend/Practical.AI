...
| `SharePointToAzureSearch.Application` | The contracts the outer layers implement. `Repositories.cs` holds the application's view of its own database — one interface per aggregate (`IChatRepository`, `IAgentRepository`, `IFileMetadataRepository`, `IDeltaStateRepository`, `IIndexStateRepository`, …); `IndexingAbstractions.cs` holds the abstractions over other people's services (`ISearchIndexStore`, `ISearchQueryStore`, `IChangeSignalPublisher`, `ISharePointChangeProcessor`, `IContentExtractor`). Plus the settings classes and orchestration that needs nothing more than those contracts. No package references. | Domain |
| `SharePointToAzureSearch.Persistence` | The Entity Framework Core model and its migrations, with one repository per contract in `Repositories/`. `AddPersistence` registers the pooled context and maps every repository interface onto its implementation, so no other project names an `Ef`-anything. | Application |
